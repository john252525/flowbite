...
<div class="flex flex-col items-center justify-center px-6 pt-8 mx-auto md:h-screen pt:mt-0 dark:bg-gray-900">
    ...
    <div class="w-full max-w-xl p-6 space-y-8 sm:p-8 bg-white rounded-lg shadow dark:bg-gray-800">
        ...
        <form class="mt-8 space-y-6" action="/u.signup" method="POST">
            @csrf
            @if ($errors->any())
            ...
            <div class="flex items-start">
                ...
                <div class="ml-3 text-sm">
                    <label for="remember" class="font-medium text-gray-900 dark:text-white">{{ __('I accept') }} <a href="/terms" class="text-primary-700 hover:underline dark:text-primary-500">{{ __('Terms and Conditions') }}</a></label>
                </div>
            </div>
            <button type="submit" class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">{{ __('Register') }}</button>
            <div class="text-sm font-medium text-gray-500 dark:text-gray-400">
                {{ __('There is an account') }}? <a href="/login" class="text-primary-700 hover:underline dark:text-primary-500">{{ __('Login') }}</a>
            </div>
        </form>
    </div>
</div>
