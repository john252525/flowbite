    ...
    <div class="w-full max-w-xl p-6 space-y-8 sm:p-8 bg-white rounded-lg shadow dark:bg-gray-800">
        ...
        <form class="mt-8 space-y-6" action="/u.login" method="POST">
            ...
            <div class="flex items-start">
                ...
                <a href="/forgot" class="ml-auto text-sm text-primary-700 hover:underline dark:text-primary-500">{{ __('Forgot the password') }}?</a>
            </div>
            <button type="submit" class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">{{ __('Login') }}</button>
            <div class="text-sm font-medium text-gray-500 dark:text-gray-400">
                {{ __('No account yet') }}? <a href="/signup" class="text-primary-700 hover:underline dark:text-primary-500">{{ __('Create an account') }}</a>
            </div>
        </form>
    </div>
</div>
